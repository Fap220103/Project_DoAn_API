...
namespace WebAPI.Controllers.Accounts
{
    public class AccountController : BaseApiController
    {
        ...
        public async Task<ActionResult<ApiSuccessResult<ResetPasswordResult>>> ResetPasswordAsync(
           ResetPasswordRequest request,
           CancellationToken cancellationToken)
        {
            var response = await _sender.Send(request, cancellationToken);

            return Ok(new ApiSuccessResult<ResetPasswordResult>
            {
                Code = StatusCodes.Status200OK,
                Message = $"Success executing {nameof(ResetPasswordAsync)}",
                Content = response
            });
        }
